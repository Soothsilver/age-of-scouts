...
namespace Age.Phases
{
    class AgeMainMenu : DoorPhase
    {
        protected override void Draw(SpriteBatch sb, Game game, float elapsedSeconds, bool topmost)
        {
            ...
            UI.DrawButton(new Rectangle(Root.ScreenWidth - 310, 400, 300, 40), topmost, "Debug mapa", () => { TransitionTo(new LevelPhase(Levels.DebugMap())); }, "Tato mapa je určena pro vývojáře.");
            UI.DrawButton(new Rectangle(Root.ScreenWidth - 310, 550, 300, 40), topmost, "Nastavení", () => { Root.PushPhase(new SettingsPhase()); }, "Změň možnosti zobrazení a další věcičky.");
            UI.DrawButton(new Rectangle(Root.ScreenWidth - 310, 600, 300, 40), topmost, "O autorech", () => { TransitionTo(new CreditsPhase()); }, "Chceš vědět, kdo na této hře pracoval?");
            UI.DrawButton(new Rectangle(Root.ScreenWidth - 310, 650, 300, 40), topmost, "Ukončit hru", () => TransitionIntoExit(), "Okamžitě ukončí hru.");
